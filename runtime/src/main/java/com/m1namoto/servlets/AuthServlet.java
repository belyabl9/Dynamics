...
import com.m1namoto.service.verification.AuthenticationTypeService;
import com.m1namoto.service.verification.VerificationTypeService;
...
public class AuthServlet extends HttpServlet {
    ...
    private static final ClassifierMakerStrategy CLASSIFIER_MAKER_STRATEGY = new WekaClassifierMakerStrategy(ClassifierType.RANDOM_FOREST);
    ...
    private void authenticate(HttpServletRequest request, HttpServletResponse response, PrintWriter out) throws IOException, ServletException {
        ...
        AuthenticationResult authResult = authenticate(context);
    ...
    private AuthenticationResult authenticate(AuthenticationContext context) {
        switch (AuthenticationTypeService.getInstance().getAuthenticationType()) {
            case IDENTIFICATION:
                return AuthenticationService.getInstance().authenticateViaIdentification(
                        context, CLASSIFIER_MAKER_STRATEGY
                );
            case VERIFICATION:
                return AuthenticationService.getInstance().authenticateViaVerification(
                        context, VerificationTypeService.getInstance().getVerificationType()
                );
            default:
                throw new UnsupportedOperationException("Specified authentication type is not supported.");
        }
    }
